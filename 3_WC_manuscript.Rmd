...
## full model
model_woodload<-lm(sqrt(woodload_m3perha) ~ reach + jam_total + reach_length_m + bankfull_width_ave_m + slope_ave_percent + val_width_ave_m + basal_area_m2perha + prop_org, data = mlr)
...
```{r}
# full model (LW only)
model_woodloadlw<- lm(sqrt(woodload_ha_lw) ~ reach + numberofjam_LW + reach_length_m + bankfull_width_ave_m + slope_ave_percent + val_width_ave_m + basal_area_m2perha, data = mlr)
...
A model was also selected for only organic jams, using all subsets selection (AICc). Only basal area (per ha) was selected in the best fit model. However, the final model is not significant (p = 0.19). 
```{r}
#full model
model_woodloadorg<- lm(sqrt(woodload_ha_org) ~ reach_length_m + numberofjam_organic + bankfull_width_ave_m + slope_ave_percent + val_width_ave_m + basal_area_m2perha, data = mlr)
...
## final model
model_woodloadorg2 <- lm(sqrt(woodload_ha_org) ~ basal_area_m2perha, data = mlr)
...
The frequency model meets all model assumptions. The full model included average jam size, reach length, bankfull width, average valley width, basal area, and percentage organic jams as predictors. A final model was chosen using all subsets selection with AICc. The final model included both basal area (per ha) and average valley width as significant predictors (p = 0.01 and 0.04, respectively). Based on Akaike weights, basal area is the more important predictor; however, the importance of both predictors is low (sum= 0.58 and 0.45 for basal area and valley width, respectively). NOTE: the best fit model includes only reach as a predictor.
```{r}
#full model
model_frequency <- lm(jam_per_ha ~ reach + jam_size_avg_m3 + reach_length_m + bankfull_width_ave_m + val_width_ave_m + basal_area_m2perha + prop_org, data = mlr)
...
#final model
freq_model_final <- lm(jam_per_ha ~ basal_area_m2perha + val_width_ave_m, data = mlr)
...
The final model for jam frequency of LW jams only also included valley width and basal area as predictors. The model is significant at $\alpha$ = 0.1 (p = 0.058). However, neither variable ranked as important during model selection (akaike weights sum = 0.51 and 0.39 for basal area and valley width, respectively). NOTE: the best fit model includes only reach as a predictor. 
```{r}
#full model
model_frequencylw <- lm(freq_LW ~ reach + jam_size_lw + reach_length_m + bankfull_width_ave_m + slope_ave_percent + val_width_ave_m + basal_area_m2perha, data = mlr)
...
#final model
model_frequencylw2 <- lm(freq_LW ~ val_width_ave_m + basal_area_m2perha, data = mlr)
...
```{r}
#full model
model_frequencyorg <- lm(freq_org ~ reach + jam_size_org + reach_length_m + bankfull_width_ave_m + slope_ave_percent + val_width_ave_m + basal_area_m2perha, data = mlr)
