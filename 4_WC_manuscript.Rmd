...
# correlation matrix for LW jams
jam_cor_LW <- select_if(filter(jam_level, jam_type == 'LW'), is.numeric) %>%
  mutate_if(is.numeric, ~replace(., is.na(.), 0))
  
jam_cor_LW <- rcorr(as.matrix(jam_cor_LW), type = 'spearman')
write.csv(jam_cor_LW$r, 'jam_r_values_LW.csv')
write.csv(jam_cor_LW$P, 'jam_p_values_LW.csv')

# correlation matrix for CPOM jams
jam_cor_CP <- select_if(filter(jam_level, jam_type == 'organic'), is.numeric) %>%
  mutate_if(is.numeric, ~replace(., is.na(.), 0))
  
jam_cor_CP <- rcorr(as.matrix(jam_cor_CP), type = 'spearman')
write.csv(jam_cor_CP$r, 'jam_r_values_CP.csv')
write.csv(jam_cor_CP$P, 'jam_p_values_CP.csv')
...
# Average Jam Count
mean(reach_level$numberofjam_LW)
mean(reach_level$numberofjam_organic)
...
h1a1 <- ggplot(data = filter(jam_level, c(jam_type == 'LW'|jam_type == 'organic')), 
       aes(x = dist_from_channel_m, y = woodvolume_m3, fill = jam_type, shape = jam_type)) + 
  geom_point(size = 3) + 
...
h1a2<- ggplot(data = filter(jam_level, c(jam_type == 'LW'|jam_type == 'organic')), 
       aes(x = ht_above_bf_m, y = woodvolume_m3, fill = jam_type, shape = jam_type)) + 
  geom_point(size =3) + 
...
h1a3 <- ggplot(data = filter(jam_level, c(jam_type == 'LW'|jam_type == 'organic')), 
               aes(x = bf_width_m, y = woodvolume_m3, fill = jam_type, shape = jam_type)) + 
  geom_point(size = 3) + 
...
ggplot(aes(x = pins, y = woodvolume_m3), data = filter(jam_level, jam_type == 'LW'))+ 
  geom_boxplot() + 
  theme_bw() + 
  labs(x = 'Number of Pins', y = bquote('LW Jam Volume' ~(m^3))) +
...
ggplot(aes(x = pins, y = woodvolume_m3), data = filter(jam_level, jam_type == 'organic'))+ 
  geom_boxplot() + 
  theme_bw() + 
  labs(x = 'Number of Pins', y = bquote('CPOM Jam Volume' ~(m^3))) +
  theme(axis.text = element_text(size = 18, color = 'black'), 
        axis.title = element_text(size = 18, color = 'black'), 
...
cor.test(jam_level$woodvolume_m3[jam_level$jam_type == 'LW'], 
         jam_level$tot_blockage[jam_level$jam_type == 'LW'])
cor.test(jam_level$woodvolume_m3[jam_level$jam_type == 'organic'], 
         jam_level$tot_blockage[jam_level$jam_type == 'organic'])

## plot of total blockage
ggplot(data = filter(jam_level, jam_type == c('LW', 'organic')), aes(x = tot_blockage, y = woodvolume_m3, fill = jam_type, shape = jam_type)) + 
  geom_point(size = 3) + 
  theme_bw() + 
  ...
  scale_shape_manual(name = 'Jam Type:', labels = c('LW', 'CPOM'), values = c(21, 24)) + 
  scale_fill_manual(name= 'Jam Type:', labels = c('LW', 'CPOM'), values = c('black', 'white')) +
  theme(legend.position = c(0.95, 0.80), axis.text = element_text(size = 10, color = 'black'), 
...
h2b1 <- ggplot(data = reach_level, aes(x = basal_area_m2perha)) +
  ...
  theme(axis.text = element_text(size = 11, color = 'black'), 
        ...
        legend.background = element_blank(),
        axis.title.x = element_blank()) + 
  scale_shape_manual(name = 'Jam Type:', labels = c('LW', 'CPOM', 'Full Reach'), values = c(21, 24, 8)) + 
  scale_fill_manual(name= 'Jam Type:', labels = c('LW', 'CPOM', 'Full Reach'), values = c('black', 'white', 'black'))
...
h2b3 <- ggplot(aes(x = basal_area_m2perha), data = reach_level) + 
  geom_point(aes(y = jam_per_ha_LW, shape = 'jam_per_ha_LW', fill = 'jam_per_ha_LW'), size = 3) + 
  geom_point(aes(y = jam_per_ha_org, shape = 'jam_per_ha_org', fill = 'jam_per_ha_org'), size = 3) +
  ylab('Jam Count per Area (jams/ha)') + 
  ...
  theme(legend.position = 'none', 
        ...
        legend.background = element_blank(), 
        axis.title.x = element_blank()) + 
  scale_fill_manual(name  ="Jam Type",
                      breaks=c("LW", "organic"),
...
h2b5 <- ggplot(aes(x = basal_area_m2perha, y = prop_org, shape = 'prop_org'), data = reach_level) + 
  geom_point(size = 3) + 
  theme_bw() + 
  ...
  theme(axis.text = element_text(size = 11, color = 'black'), 
        ...
        legend.position = 'none') + 
  scale_shape_manual(name = 'Jam Type: ', labels = c('Whole Reach'), values= c(8))
  

# plot looking at average jam size by type vs. basal area
h2b6 <- ggplot(data = reach_level, aes(x = basal_area_m2perha)) + 
  geom_point(aes(y = avg_jam_size_m3_LW, shape = 'avg_jam_size_m3_LW', fill = 'avg_jam_size_m3_LW'), size = 3) + 
  geom_point(aes(y = avg_jam_size_m3_org, shape = 'avg_jam_size_m3_org', fill = 'avg_jam_size_m3_org'), size = 3) +
...
ggarrange(plots = list(h2b1, h2b2, h2b3, h2b4, h2b6, h2b5), nrow= 3, ncol = 2, common.legend = T, legend = 'bottom', labels = c('(a)', '(b)', '(c)', '(d)', '(e)', '(f)'))

ggpubr::ggarrange(h2b1, h2b2, h2b3, h2b4, h2b6, h2b5, nrow= 3, ncol = 2, common.legend = T, legend = 'bottom', labels = c('(a)', '(b)', '(c)', '(d)', '(e)', '(f)'))
...
# full model
model_tot_woodload <- lm(sqrt(tot_woodload) ~ drainagearea_km2 + slope_mperm + RL_confinement + basal_area_m2perha + prop_org, data = reach_level)
...
## full model
model_woodload<-lm(sqrt(woodload_m3perha) ~ drainagearea_km2 + slope_mperm + RL_confinement + basal_area_m2perha + prop_org, data = reach_level)
...
```{r}
# full model
model_count <- lm(jam_total ~ drainagearea_km2 + avg_jam_size_m3 + RL_confinement + basal_area_m2perha + prop_org, data = reach_level)
...
model_count3 <- lm(jam_total ~ basal_area_m2perha + prop_org, data = reach_level)
summary(model_count3)
```
...
```{r}
#full model
model_frequency <- lm(jam_per_ha ~ drainagearea_km2 + avg_jam_size_m3 + RL_confinement + basal_area_m2perha + prop_org, data = reach_level)
